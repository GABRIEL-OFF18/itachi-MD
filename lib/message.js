...
client.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
    ...
    const isWebp = buff.slice(0, 4).toString('be') === 'RIFF'
    ...
    try {
        if (options && (options.packname || options.author)) {
            ...
            buffer = isWebp ? buff : await videoToWebp(buff)
        }
        ...
        console.error("Error al procesar sticker:", e)
        await client.sendMessage(jid, { sticker: buff, ...options }, { quoted })
        return buff
    }
}
